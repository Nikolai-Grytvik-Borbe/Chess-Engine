...
int main() {
    ...
    PieceType piece = checkSquare(&board, 1);
    printPiece(piece);
...
void initChessboard(chessBoard_t* board) {
    ...
    board->whiteKing |= (1ULL << 4) | (1ULL << 64); // (1ULL << 64) is to keep track of if the king has castled
    board->blackKing |= (1ULL << 60) | (1ULL << 64); // same as above
}
